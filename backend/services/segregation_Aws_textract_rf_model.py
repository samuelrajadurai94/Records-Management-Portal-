...
def _sanitize_text(text: str) -> str:
    """Remove NUL (0x00) characters which cause database write failures in PostgreSQL."""
    if not text:
        return ""
    return text.replace("\u0000", "")
...
def check_latest(text_list,csn_value):
    ...
    if any(any(number in page for number in [str(csn_value),"{:,}".format(int(csn_value)),
